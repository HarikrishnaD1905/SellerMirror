...
# Use add_shape instead of add_vline (Plotly's add_vline does _mean() arithmetic
# on the x-value, which breaks with date strings and pandas Timestamps)
fig_price.add_shape(
    type="line", x0=alert_date, x1=alert_date, y0=0, y1=1,
    yref="paper", line={"color": "#ff4444", "width": 2, "dash": "dash"},
)
fig_price.add_annotation(
    x=alert_date, y=1, yref="paper",
    text="Alert fired", showarrow=False,
    font={"color": "#ff4444", "size": 12},
    xanchor="left", yanchor="bottom", xshift=6,
)

# Opportunity window annotation (from alert date to end)
mid_idx  = min(alert_day_idx + (n_days - alert_day_idx) // 2, n_days - 1)
mid_date  = str(comp_prices.iloc[mid_idx]["date"]).split()[0]  # ISO string for Plotly
...
<div style="font-size: 0.9rem; color: #9ca3af; margin-top: -15px; margin-bottom: 30px;">
    <strong>Why this matters:</strong> This chart tracks the competitor's 90-day pricing strategy. Consecutive price drops (the dashed red line) 
    often signal panic discounting or excess inventory, creating a high-margin opportunity window for you to hold your price and capture their lost trust.
</div>
""", unsafe_allow_html=True)
...
st.subheader("💬 Review Sentiment — Weekly Trend")
...
fig_sent.update_layout(
    ...
    margin={"l": 0, "r": 0, "t": 10, "b": 0},
    xaxis={"showgrid": False},
    yaxis={"title": "Sentiment Score", "showgrid": True, "gridcolor": "rgba(255,255,255,0.06)",
               "range": [-1, 1]},
    legend={"orientation": "h", "yanchor": "bottom", "y": 1.02},
)
...
st.markdown("""
<div style="font-size: 0.9rem; color: #9ca3af; margin-top: -15px; margin-bottom: 30px;">
    <strong>Why this matters:</strong> This graph measures the weekly ratio of positive vs. negative words in customer reviews. 
    It reveals the actual customer experience beyond just star ratings. If your sentiment stays high while theirs drops, you are winning the Trust Gap Race—meaning your customers are much more likely to return.
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.markdown(f"**Product** &nbsp; `PHONE_CASE_01`")
